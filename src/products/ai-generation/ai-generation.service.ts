...
export class AiGenerationService {
  ...
  async generateProductDetails(
    ...
    try {
      ...
      const prompt = `You are an expert e-commerce product listing specialist. Analyze this product using the image and detailed web data to generate highly accurate, optimized details for the specified platforms. READ THE LABEL/ANY TEXT ON THE PICTURE OR DESCRIBE THE ITEM AS BEST AS YOU CAN FOR OPTIMAL SALE USING STATS (Height OR Weight). USE PRONOUNS OF THE PRODUCT IN THE TITLE IF YOU SEE ANY IN THE IMAGE (COMMON PURE EXAMPLE OF EXPECTATION: (Owala 34oz Gray Water Bottle...) just to make sure its known the cover image uploaded is the product not what that example was.)
